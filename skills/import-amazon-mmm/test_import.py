"""Contract tests for the Amazon Ads stub: it must fail, with that exact message, every time.

Run with the project venv: `.venv/bin/python skills/import-amazon-mmm/test_import.py`.
"""
...
class AmazonStubTests(unittest.TestCase):
    ...
    def test_unreadable_inputs_fail_with_one_line(self) -> None:
        utf16 = self.work / "amazon_utf16.csv"
        utf16.write_text(self.fixture.read_text(), encoding="utf-16")
        empty = self.work / "amazon_empty.csv"
        empty.write_bytes(b"")
        header_only = self.work / "amazon_header_only.csv"
        header_only.write_text(self.fixture.read_text().splitlines()[0] + "\n")
        book = self.work / "amazon.xlsx"
    ...
    def test_tracked_input_needs_the_explicit_flag(self) -> None:
        probe = REPO_ROOT / "tests" / "_guard_probe_amazon.csv"
