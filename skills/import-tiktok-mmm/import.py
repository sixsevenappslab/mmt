"""Import a TikTok MMM export into canonical form — no header contract exists yet.

TikTok Ads Manager delivers MMM data, but TikTok publishes no column list for it: the help
article says only that the file carries impressions and spend at campaign and DMA level, US
only, daily. There is nothing to map against, so this importer is a deliberate stub. It reads
and guards its input like every other importer and then stops with exit code 2. It does not
pretend to work, and it does not guess column names from third-party blog posts.
"""
...
def main() -> int:
    ...
    parser.add_argument("input_csv", type=Path, help="export CSV exactly as TikTok delivers it")
